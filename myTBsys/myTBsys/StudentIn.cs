...
namespace myTBsys
{
    public class StudentIn
    {
        ...
        private void bind(DataTable table, string fileName)
        {
            ...
            String excelStr = String.Format("select * from [学生信息$]");//这里是sheet表名  
        ...
        private void insertToSql(DataRow dataRow)
        {
            ...
            conn.Open();//打开连接
            String sqlstring = "insert into T_SH_Student(Id,Name,Password,SchoolTime,TelPhone,ClassId) "
                                 + "values('{0}','{1}','{2}','{3}','{4}','{5}')";
            try
            {
                ...
                Convert.ToString(dataRow["Id"]),
                                         ...
                                         Convert.ToDateTime(dataRow["SchoolTime"]).ToString("yyyy/MM/dd hh:mm:ss"),
                                         Convert.ToString(dataRow["TelPhone"]),
                                         Convert.ToInt16(dataRow["ClassId"])
                                         );
